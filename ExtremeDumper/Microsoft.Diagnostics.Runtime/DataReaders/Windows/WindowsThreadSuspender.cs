	...
	internal sealed class WindowsThreadSuspender : CriticalFinalizerObject, IDisposable {
				...
				do {
					...
					foreach (ProcessThread? thread in process.Threads) {
						if (thread != null) {
							...
							using var threadHandle = WindowsProcessDataReader.OpenThread(ThreadAccess.SUSPEND_RESUME, false, (uint)thread.Id);
		...
		private void ResumeThreads(IEnumerable<int> suspendedThreads) {
			foreach (int threadId in suspendedThreads) {
				using var threadHandle = WindowsProcessDataReader.OpenThread(ThreadAccess.SUSPEND_RESUME, false, (uint)threadId);
