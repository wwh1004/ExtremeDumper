...
namespace Microsoft.Diagnostics.Runtime.Utilities {
	internal unsafe class CallableCOMWrapper : COMHelper, IDisposable {
		...
		protected CallableCOMWrapper(RefCountedFreeLibrary? library, in Guid desiredInterface, IntPtr pUnknown) {
			...
			int hr = tbl->QueryInterface(pUnknown, desiredInterface, out var pCorrectUnknown);
			if (hr != 0) {
				GC.SuppressFinalize(this);
		...
		public IntPtr QueryInterface(in Guid riid) {
			var hr = _unknownVTable->QueryInterface(Self, riid, out var unk);
