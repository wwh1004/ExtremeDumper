...
namespace Microsoft.Diagnostics.Runtime
{
    ...
    {
        public ClrModule SelectedModule { get; set; }
    ...
    internal class DacDataTarget : IDacDataTarget, IMetadataLocator, ICorDebug.ICorDebugDataTarget
    {
        ...
        public unsafe int GetMetadata(string filename, uint imageTimestamp, uint imageSize, IntPtr mvid, uint mdRva, uint flags, uint bufferSize, byte[] buffer, IntPtr dataSize)
        {
            ...
            moduleHandle = (IntPtr)_dataTarget.SelectedModule.ImageBase;
            metadataAddress = _dataTarget.SelectedModule.MetadataAddress;
            metadataLength = _dataTarget.SelectedModule.MetadataLength;
            result = -1;
            if (metadataAddress == 0 || metadataLength == 0)
            {
                MemoryIO.EnumPages(_dataReader.ProcessId, moduleHandle, pageInfo =>
                {
                    byte[] pageBuffer;

                    Debug.Assert(pageInfo.Address == moduleHandle);
                    pageBuffer = new byte[pageInfo.Size];
                    if (!MemoryIO.ReadBytes(_dataReader.ProcessId, pageInfo.Address, pageBuffer))
                    {
                        Debug.Assert(false);
                        return false;
                    }
                    fixed (byte* p = &pageBuffer[0])
                        for (int i = 0; i < pageBuffer.Length / 4; i++)
                            ...
                            {
                                Buffer.BlockCopy(pageBuffer, i * 4, buffer, 0, Math.Min(buffer.Length, pageBuffer.Length - i * 4));
                                result = 0;
                                return false;
                            }
                    return true;
                ...
                result = MemoryIO.ReadBytes(_dataReader.ProcessId, (IntPtr)metadataAddress, buffer) ? 0 : -1;
            return result;
