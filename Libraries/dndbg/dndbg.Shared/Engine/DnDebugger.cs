...
namespace dndbg.Engine {
	...
	sealed class DnDebugger : IDisposable {
        ...
        public CorModuleDef GetCorModuleDef(IntPtr moduleHandle) {
            hasReceivedCreateProcessEvent = true;
            //强行设置一下，以免出现什么问题，比如无法分离进程
            foreach (DnProcess dnProcess in Processes)
            {
                dnProcess.CorProcess.RawObject.EnumerateAppDomains(out var corDebugAppDomainEnum);
                if (corDebugAppDomainEnum == null)
                    break;
                while (true)
                {
                    corDebugAppDomainEnum.Next(1, out var corDebugAppDomain, out uint count);
                    if (corDebugAppDomain == null)
                        break;
                    var dnAppDomain = dnProcess.TryAdd(corDebugAppDomain);
                    corDebugAppDomain.EnumerateAssemblies(out var corDebugAssemblyEnum);
                    if (corDebugAssemblyEnum == null)
                        break;
                    while (true)
                    {
                        corDebugAssemblyEnum.Next(1, out var corDebugAssembly, out count);
                        if (corDebugAssembly == null)
                            break;
                        var dnAssembly = dnAppDomain.TryAdd(corDebugAssembly);
                        corDebugAssembly.EnumerateModules(out var corDebugModuleEnum);
                        if (corDebugModuleEnum == null)
                            break;
                        while (true)
                        {
                            corDebugModuleEnum.Next(1, out var corDebugModule, out count);
                            if (corDebugModule == null)
                                break;
                            dnAssembly.TryAdd(corDebugModule);
                        }
                    }
                }
                foreach (DnModule dnModule in dnProcess.Modules)
                    if (dnModule.Address == (ulong)moduleHandle)
                        return dnModule.GetOrCreateCorModuleDef();
            }
            return null;
        }
